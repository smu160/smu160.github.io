...
When considering the throughput of $17.07$ $\text{Gelem/s}$, it's important to contextualize this within the hardware limits of the machine used. For instance, an AMD Ryzen 9 7950X with DDR5-6000 MHz memory can theoretically achieve memory bandwidths of up to $94.5$ $\text{GB/s}$. Given that each $\texttt{f64}$ element is $8$ bytes, the theoretical maximum throughput for this setup would be approximately:
